...
@router.get("/inbox")
async def list_tasks(org_id: int = 1, limit: int = 50, session: AsyncSession = Depends(get_session), user=Depends(require_user)) -> dict:
    try:
...
@router.post("/queue")
async def queue_task(body: dict[str, Any], session: AsyncSession = Depends(get_session), user=Depends(require_user)) -> dict:
...
async def complete_task(task_id: int, session: AsyncSession = Depends(get_session), user=Depends(require_user)) -> dict:
    rt = (await session.execute(select(ReviewTask).where(ReviewTask.id == task_id))).scalars().first()
    if not rt:
        raise HTTPException(status_code=404, detail="not found")
    try:
        require_org(user, int(rt.org_id))  # type: ignore[arg-type]
    except Exception:
        pass
    rt.status = "done"
    await session.commit()
...
async def reopen_task(task_id: int, session: AsyncSession = Depends(get_session), user=Depends(require_user)) -> dict:
    rt = (await session.execute(select(ReviewTask).where(ReviewTask.id == task_id))).scalars().first()
    if not rt:
        raise HTTPException(status_code=404, detail="not found")
    try:
...
async def post_ai_feedback(payload: dict, session: AsyncSession = Depends(get_session), user=Depends(require_user)) -> dict:
    ...
    fb = AiFeedback(
        ...
        org_id=(payload.get("org_id") or user.get("org_id") or None),
